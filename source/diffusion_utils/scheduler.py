...
# the below is from https://gist.github.com/Birch-san/230ac46f99ec411ed5907b0a3d728efa
# also found: https://gist.github.com/dvschultz/3af50c40df002da3b751efab1daddf2c
...
class ForwardDiffusionModule(GraphModuleMixin, torch.nn.Module):
    ...
    def _inference_forward(self, data:AtomicDataDict):
        ...
        if self.labels_conditioned:
            ...
            if 'target_lbl' in data:
                target_coeff = data['target_interpolation_coeff'] # must start at 0 and end at 1
                og_lbl__coeff = 1 - target_coeff
                target_lbl = data['target_lbl']
                target_cond_emb = self.labels_embedder(target_lbl)
                # labels_emb = og_lbl__coeff.to(labels_emb.device) * labels_emb + target_coeff.to(labels_emb.device) * target_cond_emb
                labels_emb = slerp(labels_emb, target_cond_emb, target_coeff.to(labels_emb.device))
